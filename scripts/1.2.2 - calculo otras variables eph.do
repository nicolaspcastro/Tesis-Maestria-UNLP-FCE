************************************ CALCULO DE TASAS DE EMPLEO - BUCLE PARA TODAS LAS EPH CEDLAS *****************************************

noi display in green "COMENZANDO DO FILE CALCULO RESTO VARIABLES EPH"


*	#NOTE - ESTE DO FILE ESTÁ DE MUESTRA, HAY QUE MODIFICARLO


quietly {
	
	
	noi display in yellow "REALIZANDO CALCULOS DE RESTO DE VARIABLES EPH"
	* Matriz de variables usando las eph puntuales por separado

    loc indicadores = ""
    loc indicadores = "pea ocupados desocupados"

    foreach indicador in `indicadores' {
        
            
        mat mat_`indicador' = J(59,28,.)
        mat colnames mat_`indicador' = "Año" "Semestre" "Encuesta" "Argentina" "Buenos Aires" "Catamarca" "Chaco" "Chubut" "Ciudad Autonoma de BA" "Cordoba" "Corrientes" "Entre Ríos" "Formosa" "Jujuy" "La Pampa" "La Rioja" "Mendoza" "Misiones" "Neuquen" "Rio Negro" "Salta" "San Juan" "San Luis" "Santa Cruz" "Santa Fe" "Santiago del Estero" "Tierra del Fuego" "Tucuman" 
        
        * Matriz de variables appendeando las eph puntuales
        
        mat mat_`indicador'_aux = J(6,28,.)
        mat colnames mat_`indicador'_aux = "Año" "Semestre" "Encuesta" "Argentina" "Buenos Aires" "Catamarca" "Chaco" "Chubut" "Ciudad Autonoma de BA" "Cordoba" "Corrientes" "Entre Ríos" "Formosa" "Jujuy" "La Pampa" "La Rioja" "Mendoza" "Misiones" "Neuquen" "Rio Negro" "Salta" "San Juan" "San Luis" "Santa Cruz" "Santa Fe" "Santiago del Estero" "Tierra del Fuego" "Tucuman" 
        ...
        * Guardo una local con todos los nombres de archivos dta de la carpeta de bases finales
        ...
        foreach encuesta in `r(files)' {
            ...
            noi display in yellow "CALCULANDO `indicador' PARA BASE `encuesta'"
            ...
            * Relleno las primeras 3 columnas de la matriz de variables, con año, semestre y encuesta
            mat mat_`indicador'[`fila',1] = `a'
            mat mat_`indicador'[`fila',2] = `s'
            if "`e'" == "ephp" mat mat_`indicador'[`fila',3] = 1	/*  EPH PUNTUAL  */
            if "`e'" == "ephc" mat mat_`indicador'[`fila',3] = 2	/*  EPH CONTINUA */

            * Cálculo el indicador para el país y lo relleno en la columna 4 de la matriz de gini
            
            qui sum `indicador' [w=pondera] if `indicador'==1 
            loc num=r(sum_w)
            qui sum `indicador' [w=pondera] 
            loc den=r(sum_w)
            loc prop=`num'/`den'
            mat mat_`indicador'[`fila',4] = `prop'

            * Calculo el indicador para cada provincia
            ...
            foreach x in `nprov' {
                ...
                loc den=r(sum_w)
                loc prop=`num'/`den'
                mat mat_`indicador'[`fila',`col'] = `prop'
        ...
        putexcel set "$path_tables\Otros EPH\vars_eph.xlsx", sheet("`indicador'", replace) modify
        putexcel A1 = matrix(mat_`indicador'), colnames
        ...
        * Hago lo mismo, pero appendeando las bases de 2003 a 2006 puntuales a las continuas
        ...
        forvalues y = 2003/2006 {
            forvalues w = 1/2 {
                ...
                noi display in yellow "CALCULANDO `indicador' AUXILIAR PARA BASE `y' `w'"

                mat mat_`indicador'_aux[`fila',1] = `y'
                mat mat_`indicador'_aux[`fila',2] = `w'
                mat mat_`indicador'_aux[`fila',3] = 3
            ...
            putexcel set "$path_tables\Otros EPH\vars_eph.xlsx", sheet("`indicador'_aux", replace) modify
            putexcel A1 = matrix(mat_`indicador'_aux), colnames
        }
    }
}
exit       
	************************************** CALCULO GINI POR AÑO (PROMEDIO DE LOS 2 SEMESTRES)
	*********** MERGE DE BASES
	...
	* Calculo diferencial para los años 2003, 2004, 2005 y 2006 donde se mezclan las encuaestas puntales y continuas
...
* Corre todo bien pero hay un problema con rio negro, me queda sin datos hasta 2002.
